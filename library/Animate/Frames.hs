...
uuu :: [String] -> Maybe [Arg]
uuu args = jjj (ddd args)

jjj :: [Either Arg String] -> Maybe [Arg]
jjj [] = Just []
jjj (a:as) = do
  a' <- qqq a
  yyy a' as

yyy :: Arg -> [Either Arg String] -> Maybe [Arg]
yyy _ [] = Just []
yyy prev (a:as) = do
  a' <- ggg prev a
  as' <- yyy a' as
...
ddd :: [String] -> [Either Arg String]
ddd args = map fff args

fff :: String -> Either Arg String
fff s
...
ggg :: Arg -> Either Arg String -> Maybe Arg
ggg _ (Left arg) = Just arg
ggg prev (Right arg) = case prev of
...
qqq :: Either Arg String -> Maybe Arg
qqq (Left arg) = Just arg
qqq _ = Nothing